const PAGES=[
  {id:'index',file:"index.html",num:'',title:'Inici',icon:'🏠'},
  {id:'dua',file:"dua.html",num:'00',title:'DUA en aquest manual',icon:'♿'},
  {id:'cap1',file:"cap1.html",num:'01',title:'Entorn de treball',icon:'🖥️'},
  {id:'cap2',file:"cap2.html",num:'02',title:'Variables i tipus de dades',icon:'📦'},
  {id:'cap3',file:"cap3.html",num:'03',title:'Condicionals',icon:'🚦'},
  {id:'cap4',file:"cap4.html",num:'04',title:'Bucles',icon:'🔄'},
  {id:'cap5',file:"cap5.html",num:'05',title:'Llistes i funcions',icon:'🧩'},
  {id:'cap6',file:"cap6.html",num:'06',title:'Sensors',icon:'🌡️'},
  {id:'cap7',file:"cap7.html",num:'07',title:'So i música',icon:'🎵'},
  {id:'cap8',file:"cap8.html",num:'08',title:'Comunicació ràdio',icon:'📡'},
  {id:'cap9',file:"cap9.html",num:'09',title:'Projectes complets',icon:'🚀'},
  {id:'cap10',file:"cap10.html",num:'10',title:'Guía pedagógica DUA',icon:'🎓'},
  {id:'cap11',file:"cap11.html",num:'11',title:'Accessibilitat i Disseny',icon:'♿'},
  {id:'apendice',file:"apendice.html",num:'A',title:'Referència ràpida',icon:'📋'},
  ];
  const SECS=[
  {title:'General',ids:['index','dua']},
  {title:'Capítols',ids:['cap1','cap2','cap3','cap4','cap5','cap6','cap7','cap8']},
  {title:'Projectes i Guia',ids:['cap9','cap10','cap11']},
  {title:'Referència',ids:['apendice']},
  ];
function buildSidebar(a){
  let h=`<div class="sidebar-logo"><div class="logo-mark">micro<span style='color:var(--coral)'>Python</span><br>a l'Aula</div><div class="logo-sub">Manual amb enfocament DUA</div></div><nav>`;
...
function buildPageNav(a){
  ...
  if(pv)h+=`<a href="${pv.file}"><span>←</span><div><span class="nav-label">Anterior</span><span class="nav-title">${pv.icon} ${pv.title}</span></div></a>`;
  else h+=`<span></span>`;
  if(nx)h+=`<a href="${nx.file}" class="nav-next"><div><span class="nav-label">Següent</span><span class="nav-title">${nx.icon} ${nx.title}</span></div><span>→</span></a>`;
  h+='</div>';return h;
}
function initPage(a){
...
function duaBox(type,items){
  const cfg={rep:{cls:'dua-rep-group',icon:'🟥',label:'DUA — Representació'},ace:{cls:'dua-ace-group',icon:'🟦',label:'DUA — Acció i Expressió'},imp:{cls:'dua-imp-group',icon:'🟩',label:'DUA — Implicació'}}[type];
...
function noteBox(i){return box('note','📝','Nota per al docent',i);}
function tipBox(i){return box('tip','💡','Consell didàctic',i);}
function warnBox(i){return box('warn','⚠️','Atenció',i);}
function conceptBox(i){return box('concept','🔑','Concepte clau',i);}
...
const LVL=['', '🟢 Iniciació (6-8)', '🟡 Bàsic (9-11)', '🟠 Intermedi (12-13)', '🔴 Avançat (14-16)'];
