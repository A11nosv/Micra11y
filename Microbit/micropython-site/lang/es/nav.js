...
function buildSidebar(a){
  let h=`<div class="sidebar-logo"><div class="logo-mark">micro<span style='color:var(--coral)'>Python</span><br>en el Aula</div><div class="logo-sub">Manual con enfoque DUA</div></div><nav>`;
...
function buildPageNav(a){
  ...
  if(nx)h+=`<a href="${nx.file}" class="nav-next"><div><span class="nav-label">Siguiente</span><span class="nav-title">${nx.icon} ${nx.title}</span></div><span>→</span></a>`;
  h+='</div>';return h;
}
function initPage(a){
...
function duaBox(type,items){
  const cfg={rep:{cls:'dua-rep-group',icon:'🟥',label:'DUA — Representación'},ace:{cls:'dua-ace-group',icon:'🟦',label:'DUA — Acción y Expresión'},imp:{cls:'dua-imp-group',icon:'🟩',label:'DUA — Implicación'}}[type];
...
function noteBox(i){return box('note','📝','Nota para el educador',i);}
function tipBox(i){return box('tip','💡','Consejo didáctico',i);}
