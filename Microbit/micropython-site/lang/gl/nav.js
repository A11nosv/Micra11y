const PAGES=[
  ...
  {id:'dua',file:"dua.html",num:'00',title:'DUA neste manual',icon:'♿'},
  {id:'cap1',file:"cap1.html",num:'01',title:'Contorno de traballo',icon:'🖥️'},
  {id:'cap2',file:"cap2.html",num:'02',title:'Variables e tipos de datos',icon:'📦'},
  {id:'cap3',file:"cap3.html",num:'03',title:'Condicionais',icon:'🚦'},
  {id:'cap4',file:"cap4.html",num:'04',title:'Bucles',icon:'🔄'},
  {id:'cap5',file:"cap5.html",num:'05',title:'Listas e funcions',icon:'🧩'},
  {id:'cap6',file:"cap6.html",num:'06',title:'Sensores',icon:'🌡️'},
  {id:'cap7',file:"cap7.html",num:'07',title:'Son e musica',icon:'🎵'},
...
function buildSidebar(a){
  let h=`<div class="sidebar-logo"><div class="logo-mark">micro<span style='color:var(--coral)'>Python</span><br>na Aula</div><div class="logo-sub">Manual con enfoque DUA</div></div><nav>`;
...
function buildPageNav(a){
  ...
  if(nx)h+=`<a href="${nx.file}" class="nav-next"><div><span class="nav-label">Seguinte</span><span class="nav-title">${nx.icon} ${nx.title}</span></div><span>→</span></a>`;
  h+='</div>';return h;
}
function initPage(a){
...
function duaBox(type,items){
  const cfg={rep:{cls:'dua-rep-group',icon:'🟥',label:'DUA — Representación'},ace:{cls:'dua-ace-group',icon:'🟦',label:'DUA — Acción e Expresión'},imp:{cls:'dua-imp-group',icon:'🟩',label:'DUA — Implicación'}}[type];
...
function noteBox(i){return box('note','📝','Nota para o docente',i);}
function tipBox(i){return box('tip','💡','Consello didáctico',i);}
function warnBox(i){return box('warn','⚠️','Atención',i);}
function conceptBox(i){return box('concept','🔑','Concepto clave',i);}
...
const LVL=['', '🟢 Iniciación (6-8)', '🟡 Básico (9-11)', '🟠 Intermedio (12-13)', '🔴 Avanzado (14-16)'];
